...
We can use a specific graphic to visualize the coefficients and associated standard errors


```{r visualregIni}

plot_summs(reglin, scale = FALSE,
...
##  Criteria for finding the right number of variables
...
ggplot(leapF)+
  ...
  scale_x_continuous( breaks =seq(0, rcvTunes, 2)) + 
...
We can compare the penalization models in terms of lowest RMSE, for all Cross Validated samples. 

```{r PenalComparison}
models <- list( lm = lm, cp = lmcp, forward = leapF, 
                backward = leapB, stepwise = leapS, 
                ridge = ridge, lasso = lasso, enet = enet)
perf <- resamples(models)
colvec <- c('#ffffcc', SIAP.color, "pink")
boxplot(perf$values[c("ridge~RMSE", "lasso~RMSE", "enet~RMSE")],
        #names = names(models),
...
LASSO - and elastic net since here elastic net reduces to LASSO in this example - seems to perform better than ridge regression, with almost the same set of  variables. 
...
models <- list( lm = lm, cp = lmcp, forward = leapF, 
                ...
                ridge = ridge, lasso = lasso, enet= enet)
perf <- resamples(models)
colvec <- c('#fbb4ae','#b3cde3','#ccebc5','#decbe4','#fed9a6','#ffffcc', SIAP.color,"pink")
boxplot(perf$values[c("lm~RMSE", "cp~RMSE", 
                      "forward~RMSE", "backward~RMSE", 
                      "stepwise~RMSE", "ridge~RMSE", "lasso~RMSE", "enet~RMSE")],
        #names = names(models),
        names = c("lm", "Cp", "forwd", "backwd", "step", "ridge", "lasso", "e-net"),
        col=colvec,horizontal = FALSE,
        ylab="RMSE",
        frame.plot = FALSE)
``` 

In this example, it seems that penalization methods performed quite well, even with a large number of variables.  $C_p$ is doing almost the same job with less variables. 
