...
It is important to check if the distribution of the the variable of interest (*Before15*) is the same in the training and the validation (test) data sets. 

```{r fig.height=2, fig.width= 8}
ggplot(train_data) + geom_bar(aes(y = Before15), colour="white", fill = SIAP.color) +
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5))+ 
  labs(x = "", y = "child marriage") +
  ggtitle("Marriage before 15 - train ")
```

```{r fig.height=2, fig.width= 8}
ggplot(validation_data) + geom_bar(aes(y = Before15), colour="white", fill = SIAP.color) +
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5))+ 
  labs(x = "", y = "child marriage") +
  ggtitle("Marriage before 15 - validation ")
```
...
We therefore begin by tuning the **number of possible variables** considered at each split in a decision tree by tuning the *mtry* hyperparameter. We do a grid search and compute the accuracy for each value of *mtry* in a suitable interval ($\in \{0,8\}$ here). Note that this grid search should be limited if the number of variables is lower than 8!


```{r, cache = TRUE}
# /!\ running this chunk can be long!
# Defining the grid search on mtry
# NB: We limit the grid search to the number of explanatory variables 
