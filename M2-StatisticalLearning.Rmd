...
## Tools
We will use several packages, in particular the *caret* package https://topepo.github.io/caret/index.html.
Caret can deal with 238 "models": we are going to use some of them. 

Here is the list of packages we will use: 
...
# Introduction
This file serves as support for the "*Workshop on Machine Learning for Official Statistics in Asia-Pacific*"  

The term *machine learning* is used because the computer (more accurately the algorithm) figures out the model from the data and will then predict with accuracy. On the other hand,  *statistical learning* is mostly used to test whether a model fits the data and to showcase the relationships between variables. 
...
ggplot(Singles, aes(x = ltexp, y = FoodShr)) +
  ...
  geom_line(aes(y = predictions$fit), color = "blue", size = 0.7) +  # Regression line
...
boxplot(res.all, ylim = c(-0.5, 0.5),outline=FALSE,   
        frame.plot = FALSE, horizontal = FALSE,
