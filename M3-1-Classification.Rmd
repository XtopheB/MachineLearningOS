---
title: "Supervized Classification"
author: "Christophe Bontemps & Patrick Jonsson - SIAP^[*This document uses teaching
  materials developped by Pascal Lavergne* (*Toulouse School of Economics*)]"
...
library(knitr)
# Chunk option to toggle output on/off
knitr::opts_chunk$set(echo = TRUE)
```


```{r Knitr_Global_Options, include=FALSE, cache=FALSE}
# Global options for document rendering
...
Let's start with an example and a data set with households  living in a *rural* or *urban* environment. 
To try to characterize (and then predict) each household location (our *dependent* variable) , we have two predictors (or *explanatory* variables)  *Education* and *Income.
...
A quick summary showing the repartition of household's locations.  

```{r summary}
summary(twoClass$classes)
```

Note that we have defined *Urban* as being the first class.
...
## A first classifier in one dimension
A classifier typically separates **urban** from **rural** households using a simple threshold. Here for example, the classifier is a separating households based on the level of education with a threshold at 0.3. On the right of this threshold, households are in majority *Urban* , on the left *Rural*.     
...
ggplot(data = twoClass,aes(x = Education, y= 1)) + 
  geom_point(aes(color = classes), size = 2, alpha = .5) +
  geom_segment(aes(x = 0.3, y = 0.5, xend = 0.3, yend = 1.5),
               colour = "orange", 
               linewidth = 2)+
 # annotate("text", x = 0.3,  label = "urban")
  scale_colour_manual(name = 'classes', 
                      values = twoClassColor) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0.9,1.1))+
  theme_minimal()+ 
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank())
...
## Two-dimensions classifier
 Since we have **two** explanatory variables, we can represent our households with a scatterplot along the two explanatory variables "*Education*" and " *Income*".
...
## This chuk is quite technical and is not useful for you
nbp <- 250  # Number of points for prediction
...
# Function for representing decisions boundaries for any classifier 
...
### Parameters
All over this exercise, we will use the same sets of parameters consistently. This includes the parameters that will ensure reproducibility by specifying the seeds used in the random number generation. 
...
```{r sixtats}
# A function that recalls always the same 6 statistics 
sixStats <- function(...) c(twoClassSummary(...), 
                            defaultSummary(...))
```


The parameters specific to training the Machine Learning models are recorded in the object `ctrl`

- **number** = K: Defines the number of folds in k-fold cross-validation.
If K = 5, the data set is split into 5 subsets; 4 are used for training, and 1 is used for validation (80-20 rule). We repeat K-times

- **repeats**: Specifies how many times the k-fold cross-validation should be repeated (sometimes named **m**)

In total we'll have $k \times m$ (train-validation) samples
...
seed <- 123
# For repeated cross validation, we use the function setSeeds created above
...
```{r}
# New control variables including a search on different models 
K <- 5
repeats <- 10
rcvTunes <- 1 # tune number of models
